...
export const getInputBorderRadius = node => {
    return node.type === 'radio' ? INPUT_RADIO_BORDER_RADIUS : INPUT_CHECKBOX_BORDER_RADIUS;
};

export const inlineInputElement = (node, container) => {
...
export const inlineTextAreaElement = (node, container) => {
    inlineFormElement(node.value, node, container, true);
};

export const inlineSelectElement = (node, container) => {
    const option = node.options[node.selectedIndex || 0];
    inlineFormElement(option ? option.text || '' : '', node, container, false);
};

export const reformatInputBounds = bounds => {
